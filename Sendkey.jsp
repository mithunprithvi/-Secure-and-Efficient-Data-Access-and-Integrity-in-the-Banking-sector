...
PreparedStatement p = d.prepareStatement("SELECT Filename,Filekey FROM bank.upload");
...
<%--  <td><%=s.getString(4)%></td> 
  <td><%=s.getString(5)%></td> 
  <td><%=s.getString(6)%></td>   --%>
<td align="left"><a href="MailSend?name=<%=name%>&&filename=<%=filename%>&&filekey=<%=filekey%>&&user=<%=username%>"><button class="btn btn-danger" style="font-size:15px; text-align:center;">Send Key</button></a></td>
</tr> 
  <%}%>  
</table> 
</body>
